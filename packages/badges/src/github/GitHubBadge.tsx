...
export interface GitHubBadgeProps extends Omit<SocialBadgeProps, 'platformName' | 'colors' | 'logoComponent'> {
...
export const GitHubBadge: React.FC<GitHubBadgeProps> = ({
  ...
  ...props
    ...
    // GitHub-specific colors for light and dark themes
    const gitHubColors = {
        light: {
            border: '#24292e20',
            text: '#24292e',
    ...
    const colors = theme === 'dark' ? gitHubColors.dark : gitHubColors.light;

    // GitHub logo with theme-specific fills
    const logoFills = theme === 'light'
        ? { path: '#24292e' }
        : { path: '#FFFFFF' };

    const gitHubLogo = (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 34 34" width="31" height="31" focusable="false">
            <g transform="scale(1.4)" fill="none" fillRule="evenodd">
                <path
